    ...
    <v-app :class="getScreen">
        <v-content>
            <v-container v-scroll="onScroll" class="homepage" style="padding:0px;" fluid>
                <v-navigation-drawer class="pl-drawer" style="background-color: rgba(0, 0, 0, 0.76);"
                    disable-resize-watcher disable-route-watcher fixed v-model="drawer" app>
                    <v-list dense>
                        <v-list-tile>
                            <v-list-tile-action>
                    ...
                    <v-flex xs12 sm12 md12>
                        <v-toolbar :class="transparentNav" class="peer-toolbar" fixed dark style="padding-top: 5px;">
                            <v-toolbar-side-icon @click="drawer = !drawer" style="position: fixed;"
                                v-show="!$vuetify.breakpoint.mdAndUp"></v-toolbar-side-icon>
                            <v-toolbar-title v-show="$vuetify.breakpoint.mdAndUp" style="font-weight:100;"
                                class="white--text">Auction Dapp</v-toolbar-title>
                            <v-spacer></v-spacer>
                ...
                <v-card>
                    ...
                    <v-list v-show="!networkReady" three-line subheader>
                        <v-subheader style="color:red;">
                            ...
                            Error: Please make sure metamask is installed and connected to a network with an account
                            selected
                        </v-subheader>
                    </v-list>
                    <v-list v-show="networkReady" three-line subheader>
                        ...
                        <v-stepper v-model="stepperIndex" vertical>
                                ...
                                <v-card class="mb-5">
                                    <v-layout xs12 sm12 md12 row wrap>
                                        <v-flex style="height:100%; padding-bottom:20px;" xs12 sm12 md12>
                                            ...
                                            <v-btn v-show="!createAssetSuccess"
                                                :disabled="deed.deedURI.length == 0 || creatingAsset" color="teal"
                                                outline @click.native="registerDeed()">Create Deed/Token</v-btn>
                                            <!-- <v-btn v-show="!createAssetSuccess" outline color="teal" @click="stepperIndex++">Skip</v-btn> -->
                                        </v-flex>
                                    </v-layout>
                                </v-card>
                                <v-btn v-show="createAssetSuccess" outline color="primary" @click="stepperIndex++"
                                    flat>Next</v-btn>
                            </v-stepper-content>
                            ...
                            <v-stepper-content step="2">
                                <v-card class="mb-5">
                                    <v-layout xs12 sm12 md12 row wrap>
                                        <v-flex style="height:100%; padding-bottom:20px;" xs12 sm12 md12>
                                            <v-flex style="height:100%; padding-bottom:20px;" xs12 sm12 md12>
                                                <v-select v-model="selectedDeed" :items="deeds"
                                                    label="Asset"></v-select>
                                            </v-flex>
                                            <div v-show="transferDeedSuccess">
                                                <v-icon color="teal">check_circle</v-icon>
                                                <h3 style="color:green !important;">Ownership was successfully
                                                    transfered</h3>
                                            </div>
                                            ...
                                            <v-btn
                                                :disabled="selectedDeed == null || transferDeedSuccess || transferingDeed"
                                                @click="transferTo()" outline color="teal">Transfer Ownership</v-btn>
                                            <v-btn v-show="transferDeedSuccess" outline color="primary"
                                                @click="stepperIndex++">Next</v-btn>
                            ...
                            <v-stepper-content step="3">
                                <v-card class="mb-5">
                                    <v-layout xs12 sm12 md12 row wrap>
                                        <v-flex style="height:100%; padding-bottom:20px;" xs12 sm12 md12>
                                            <v-select v-model="auction.deedId" :items="deeds"
                                                label="Deed ID"></v-select>
                                        </v-flex>

                                        <v-flex style="height:100%; padding-bottom:20px;" xs12 sm12 md12>
                                            <v-text-field v-model="auction.auctionTitle" placeholder="e.g. My NFT"
                                                label="Auction title" persistent-hint></v-text-field>
                                        </v-flex>

                                        <v-flex style="height:100%; padding-bottom:20px;" xs12 sm12 md12>
                                            <b style="color:red;">Image(png/jpg):</b> <input type="file"
                                                @change="fileSelectionEvent($event)">
                                        </v-flex>

                                        <v-flex style="height:100%; padding-bottom:20px;" xs12 sm12 md12>
                                            <v-text-field v-model="auction.startingPrice" placeholder="10 Ethers"
                                                label="Starting Price" persistent-hint></v-text-field>
                                        </v-flex>
                                        <!-- <v-flex  style="height:100%; padding-bottom:20px;" xs12 sm12 md12>
                                        <v-text-field
                                        ...
                                        <v-flex style="height:100%; padding-bottom:20px;" xs12 sm6 md6>
                                            <v-btn @click="uploadAndCreateAuction()" outline color="teal">Create
                                                Auction</v-btn>
                                        </v-flex>
                                    </v-layout>
                                </v-card>
            ...
            <v-fab-transition>
                <v-btn color="teal" dark fixed bottom right fab v-show="!statusPopup"
    ...
    methods: {
        async transferTo() {
            try {
                ...
                await this.$deedRepoInstance.transferTo(this.$config.AUCTIONREPOSITORY_ADDRESS, this.selectedDeed)
        ...
        /**
         * uploads the metadata of an auction to swarm and executes the smart contract
         */
        ...
        async registerDeed() {
            try {
                ...
                this.$deedRepoInstance.setAccount(this.getWeb3DefaultAccount)
                await this.$deedRepoInstance.create(this.deed.deedId, this.deed.deedURI)
