<template>
    <div>
        <v-layout style="background: linear-gradient(to right, rgb(29, 52, 70), rgb(25, 28, 31)); padding:0px;"
            justify-center align-center row wrap>
        ...
        <v-layout wrap row v-for="auc in auction" :key="auc.id">
            <v-flex d-flex xs12 sm12 md8 style="">
                <div
                    ...
                    <v-flex d-flex xs12 sm12 md12 style="text-align:left; margin-top:7px;">
                        <h3><v-icon dark>account_balance_wallet</v-icon> <b>Starting price: </b> {{ auc.startingPrice }}
                            ETH</h3>
                    </v-flex>
                    <v-flex d-flex xs12 sm12 md12 style="text-align:left; margin-top:7px;">
                        <h3><v-icon dark>account_balance_wallet</v-icon> <b>Current bid: </b> {{ auc.lastBidAmount }}
                            ETH
                        </h3>
                    </v-flex>
                    <v-flex d-flex xs12 sm12 md12 style="text-align:left; margin-top:7px;">
                    ...
                    <v-flex d-flex xs12 sm12 md12 style="text-align:left; margin-top:7px;">
                        <h3><v-icon dark>date_range</v-icon> Expires on: <b>{{ auc.expirationDate }}, {{ auc.timeLeft }}
                                remaining</b></h3>
                    </v-flex>
                    <v-flex d-flex xs12 sm12 md12 style="text-align:left; margin-top:7px;">
                    ...
                    <div style="margin-top:13px;">
                        <v-btn :disabled="isAuctionOwner(auc) || !auc.active" @click="bidModal = true"
                            style=" margin:0; width:100%;" color="teal" dark> Bid now </v-btn>
                        <span style="color:red;" v-show="isAuctionOwner(auc)">You can't bid on your own auctions</span>
                    </div>
                    <div style="margin-top:13px;">
                        <v-btn @click="cancelAuction(auc.id)" :disabled="!isAuctionOwner(auc) || !auc.active"
                            style=" margin:0; width:100%;" color="teal" dark> Cancel Auction </v-btn>
                    </div>
                    <div style="margin-top:13px;">
                        <v-btn @click="finalizeAuction(auc.id)" :disabled="!isAuctionOwner(auc) || !isLastBidder(auc)"
                            style=" margin:0; width:100%;" color="teal" dark> Finalize Auction </v-btn>
                    </div>
                </div>
            </v-flex>
        ...
        <v-layout style=" color: white; padding: 25px; text-align: center;     background-color: rgb(3, 46, 66);"
            ...
            <v-flex style="height:100%; padding-bottom:20px;" xs12 sm12 md12>
                ...
                <div v-show="!joined" style="width: 60%; margin: 0 auto;">
                    ...
                    <v-btn @click="joinRoom()" :disabled="loading" outline color="white"> <v-progress-circular
                            v-show="loading" indeterminate v-bind:size="20" color="teal"></v-progress-circular>
                        Join</v-btn>
                </div>
            </v-flex>
        </v-layout>
                    ...
                    <v-list three-line>
                        ...
                        <template v-for="(item, i) in messages">
                            ...
                            <v-list-tile avatar v-bind:key="item.data">
                                ...
                                <v-list-tile-content>
                                    <v-list-tile-title style="font-size:14px; color:gray;"
                                        v-html="item.identity"></v-list-tile-title>
                                    <v-list-tile-sub-title style="font-size:17px; color:black;"
                                        v-html="item.data"></v-list-tile-sub-title>
                                </v-list-tile-content>
                            </v-list-tile>
                        </template>
                        <v-list-tile avatar>
                            ...
                            <v-list-tile-content>
                                <v-list-tile-sub-title>
                                    <v-text-field id="compose" v-model="myMessage" label="Click here to type a message"
            ...
            <v-card>
                ...
                <v-card-text style="text-align: center;">
                    <v-flex style="height:100%; padding-bottom:5px;" xs12 sm12 md12>
                        <v-text-field v-model="bidPrice" placeholder="e.g. 0.6" label="Bid amount"
